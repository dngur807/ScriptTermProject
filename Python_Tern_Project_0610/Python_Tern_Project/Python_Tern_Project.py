...
#http://cosmosjs.blog.me/220980842658
...
#from PyQt5.QtWidgets import QGridLayout, QLabel, QLineEdit , QStyle
...
class MainWindow(QDialog, UI_Main.UI_Image):
    def __init__( self , parent = None):
        super(MainWindow, self).__init__(parent)
         ...
         #날씨 클래스
        self.WeatherInfo = Weather_Main.Weather();     
        ...
        self.provinceList =  self.WeatherInfo.provinceList;
        self.cityDic = self.WeatherInfo.cityDic;
       ...
       # self.ParseAirpollution();#파싱
        ...
        self.provincecomboBox.setCurrentIndex(-1);
        ...
        self.provincecomboBox.currentIndexChanged.connect(self.WProvince_Select , self.provincecomboBox.currentIndex());               
        ...
        self.WpushButton.clicked.connect(self.WeatherShow);
        self.WeatherSendButton.clicked.connect(self.WeathersendEmail);
        self.Time_Label.setText(self._translate("Time_Label" ,self.WeatherInfo.Time));
...
# 버튼 클릭 지역 검색
    def ParseAirpollution(self , index):
        ...
        #온도 
        #temp = Weather_Func.getvalue_weather (Weather_Func.Parse_weather('weather' , 'current' , 'minutely' , index) , "temperature")["tc"];
        #print(temp);
    ...
    def WProvince_Select(self , Index) :
        self.provinceName = self.provinceList[Index];
        self.citycomboBox.clear();
        self.cityList.clear();
        for v , item in self.cityDic.items() :
            if  self.provinceName == item :
                self.cityList.append(v);
                self.citycomboBox.addItem(v)
            
        self.datacomboBox.clear();
        self.DataList.clear();

        for i in range(len(self.Weather_Result)) :
            for item in self.Weather_Result[i] : 
                if self.provinceName == item  and self.cityList[self.citycomboBox.currentIndex()] == self.Weather_Result[i][1]:
                    self.DataList.append(self.Weather_Result[i][3]);
                    self.datacomboBox.addItem(self.Weather_Result[i][3]);

    def Station_Select(self , Index) :
        self.stationName = self.root[1][0][Index][0].text;
    def WeatherShow(self):
        if ( self.datacomboBox.currentIndex() == -1 or 
self.citycomboBox.currentIndex() == -1
or self.provincecomboBox.currentIndex() == -1) :
            return;
        self.cityName = self.cityList[self.citycomboBox.currentIndex()];
        self.DataInfo = self.DataList[ self.datacomboBox.currentIndex()];
        for i in range(len(self.Weather_Result)) :
            if self.Weather_Result[i][0] == self.provinceName and self.Weather_Result[i][1] == self.cityName and self.Weather_Result[i][3] == self.DataInfo :
                # 정보 셋팅
                self.WeatherBrowser.setText(self._translate("WeatherBrowser", self.Weather_Result[i][4]));# 날씨
                self.MinTemBrowser.setText(self._translate("MinTemBrowser", self.Weather_Result[i][5]));# 최저 기온
                self.MaxTemBrowser.setText(self._translate("MaxTemBrowser", self.Weather_Result[i][6]));# 최대 기온
                self.reliBrowser.setText(self._translate("reliBrowser", self.Weather_Result[i][7]));# 신뢰도
                self.WeatherCommentBrowser.setText(self._translate("WeatherCommentBrowser", self.WeatherInfo.CommentString()));# 코멘트

                if self.Weather_Result[i][4] == "구름조금":
                   writeImageWidget(self.Weather_img,"/Resource/","02","png")
                   self.Weather_img.setScaledContents(True)
                elif self.Weather_Result[i][4] == "구름많음":
                   writeImageWidget(self.Weather_img,"/Resource/","03","png")
                   self.Weather_img.setScaledContents(True)
                elif self.Weather_Result[i][4] == "맑음":
                   writeImageWidget(self.Weather_img,"/Resource/","01","png")
                   self.Weather_img.setScaledContents(True)
                elif self.Weather_Result[i][4] == "구름많고 비":
                   writeImageWidget(self.Weather_img,"/Resource/","13","png")
                   self.Weather_img.setScaledContents(True)
                  #  writeImageWidget(self.Weather_img,"/Resource/","good2","png")
                   # self.Weather_img.setScaledContents(True)

                #메일로 보낼 정보
                s = ""
                s += "====== <기상 정보> =======\n<발표 시각> :" + self.WeatherInfo.Time + "\n<기상 전망> :" + self.WeatherInfo.CommentString()  + "\n<지역> : " + self.provinceName + "\n<도시> :" + self.cityName   + "\n<날씨> :" +  self.Weather_Result[i][4] + "\n<기온> : " + self.Weather_Result[i][5] + " / " + self.Weather_Result[i][6] + "\n<신뢰도> : "  + self.Weather_Result[i][7] + "\n";
                print(s);
                self.currentInfo = s;
...
self.sidocomboBox.currentIndex() == -1) :
            return;
        #temp = Weather_Func.getvalue_weather (Weather_Func.Parse_weather('weather' , 'current' , 'minutely' , self.sidoName , self.stationName) , "temperature")["tc"];
        #print(temp);
        #현재 시간 입력
    ...
    def WeathersendEmail(self):#미세먼지
        #self.set
        if (self.Send_email_ID == "" or self.Send_email_Psd == ""
            or self.Recv_email_ID == ""):
            self.WeatherCheckBrowser.setText("메일 전송 실패");
            return None;
        ...
        self.WeatherCheckBrowser.setText("메일 전송 완료");
        s.close();
